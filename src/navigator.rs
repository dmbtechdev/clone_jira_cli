    ...
    pub fn handle_action(&mut self, action: Action) -> Result<()> {
        match action {
            ...
            Action::CreateEpic => {
                ...
                self.db.create_epic(new_epic).with_context(|| anyhow!("failed to create epic!"))?;
            }
            Action::UpdateEpicStatus { epic_id } => {
                ...
                if let Some(new_status) = (self.prompts.update_status)() {
                    self.db.update_epic_status(epic_id, new_status).with_context(|| anyhow!("failed to update epic status!"))?;
                }
            }
            ...
            Action::UpdateStoryStatus { story_id } => {
                // todo!() // prompt the user to update status and persist it in the database
                if let Some(new_status) = (self.prompts.update_status)() {
                    self.db.update_story_status(story_id, new_status).with_context(|| anyhow!("failed to update story status!"))?;
                };
            }
