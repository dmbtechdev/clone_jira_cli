...
impl Page for HomePage {
    fn draw_page(&self) -> Result<()> {
        ...
        for (id,epic) in self.db.read_db()?.epics.iter().sorted() {
            println!("{:12}|{:34}|{:17}", get_column_string(&id.to_string(),12), get_column_string(&epic.name,34), get_column_string(&epic.status.to_string(),17));
        }
    ...
    fn handle_input(&self, input: &str) -> Result<Option<Action>> {
        ...
        //     _ if input.chars().any(|c| c.is_ascii_digit()) => {
        //         match input.parse() {
        //             Ok(epic_id) => {
            ...
            input => {
                if let Ok(epic_id) = input.parse::<u32>() {
                    if epics.contains_key(&epic_id) {
                        ...
                        return Err(anyhow!("Epic no:{} is not available", input));
                    }
                }
                Ok(None)
            }
...
impl Page for EpicDetail {
    fn draw_page(&self) -> Result<()> {
        ...
        println!("{:6}|{:14}|{:29}|{:14}", get_column_string(&self.epic_id.to_string(),6), get_column_string(&epic.name,14), get_column_string(&epic.description,29), get_column_string(&epic.status.to_string(),14));
        ...
        for id in epic.stories.iter().sorted() {
            println!("{:12}|{:34}|{:17}", get_column_string(&id.to_string(),12), get_column_string(&stories[id].name,34), get_column_string(&stories[id].status.to_string(),17));
        }
            ...
            input => {
                if let Ok(story_id) = input.parse::<u32>() {
                    if epic.stories.contains(&story_id) {
                        ...
                        return Err(anyhow!("Story no:{} is not available", input));
                    }
                }
                Ok(None)
            }
...
impl Page for StoryDetail {
    fn draw_page(&self) -> Result<()> {
        ...
        println!("{:6}|{:14}|{:29}|{:15}", get_column_string(&self.story_id.to_string(),6), get_column_string(&story.name,14), get_column_string(&story.description,29), get_column_string(&story.status.to_string(),15));
